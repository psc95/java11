...
public class A {//외부 클래스 A
	...
	public class B{
		...
		int field01;//인스턴스 변수
		//static int field02;//정적변수는 올수 없다.
		void method01() {}//인스턴스 메서드 올수 있다.
		//static void method02() {}//정적메서드 올수 없다.
	}//B class
	
	/* 정적 멤버 내부 클래스 */
	static class C{
		/* 정적 멤버 내부클래스에는 인스턴스변수,인스턴스 메서드,정적변수,정적메서드 모두 올수 있다.  
		 */
		C(){
	...
	void method() {
		/* 로컬 지역클래스 : 외부클래스$1내부클래스명.class (A$1D.class)
		 */
		class D{
			D(){System.out.println("D생성자 호출");	}
			
			/* 로컬 지역클래스에는 인스턴스 변수와 인스턴스 메서드는 올 수 있다.
			 * 정적변수와 정적메서드는 올수 없다.
			 */
			int field01;
			//static int field02;

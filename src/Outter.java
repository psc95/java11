/* 메서드 호출이 끝이 나면 메서드 소속의 매개변수나 지역변수는 스택 메모리에서 사라진다.하지만 해당 메서드 내부의 로컬 지역
 * 클래스 객체는 메서드 호출이 끝이 나도 메모리에 남아 있다.로컬 지역 내부 클래스에서 메서드의 매개변수나 지역변수를 복사를 
 * 해놓고 사용하다 보니 값 불일치 현상이 발생할 수 있다.결국 로컬 지역 내부에서는 매개변수나 지역변수가 메모리에 남아 있어서 값을
 * 변경하는 경우가 발생하면 값 불일치 현상이 발생하기 때문에 메서드 소속의 매개변수나 지역변수는 값 수정을 못하게 jdk 1.7이전
 * 에서는 final상수로 선언해서 값 수정을 못하게 하고,jdk 1.8이후부터는 final을 묵시적으로 생략해도 상수로 인식되어서 값을
 * 수정할 수가 없다.
 */
public class Outter {
